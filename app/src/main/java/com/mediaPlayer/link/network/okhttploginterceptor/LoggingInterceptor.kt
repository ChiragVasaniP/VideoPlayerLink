package com.mediaPlayer.link.network.okhttploginterceptor

import android.util.Log
import com.google.gson.Gson
import com.google.gson.GsonBuilder
import com.mediaPlayer.link.network.okhttploginterceptor.printer.DefaultLogPrinter
import com.mediaPlayer.link.network.okhttploginterceptor.printer.IPrinter
import okhttp3.*
...
class LoggingInterceptor @JvmOverloads constructor(
    ...
    Interceptor {
    private var requestTag = "$tag-$REQUEST"
    private var responseTag = "$tag-$RESPONSE"
    ...
    private fun printRequest(r: Request) {
        ...
        requestBody?.also { itRequest ->
            if (itRequest is MultipartBody) {
                list.add("$L Multipart: size=${itRequest.parts.size}")
                itRequest.parts.mapIndexed { i, part ->
                    val body = part.body
